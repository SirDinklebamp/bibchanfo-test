...
function renderPagination() {
  ...
  const makeButton = (label, page, disabled = false, current = false) => {
    ...
    button.addEventListener("click", () => { currentPage = page; render(); window.scrollTo({top: document.getElementById("results").offsetTop - 20, behavior:"smooth"}); });
    return button;
  };
  nav.appendChild(makeButton("Précédent", currentPage - 1, currentPage === 1));
  for (let p = 1; p <= pages; p++) nav.appendChild(makeButton(String(p), p, false, p === currentPage));
